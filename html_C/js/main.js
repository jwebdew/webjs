$(function () {
    ...
    $('.cont01 .item').mouseenter(function(){
        //.item에게 마우스 올렸을 때 실행
        //마우스올린 .item의 자식인 img의 src 가져옴
        let thisImg = $(this).find('img').attr('src');
        console.log(thisImg)

        //마우스올린 .item의 자식인 h4의 텍스트 가져옴
        let thisTextTitle = $(this).find('h4').text();
        let thisText = $(this).find('p').text();

        $('.big_image img').attr('src', thisImg);
        $('.big_image h4').text(thisTextTitle);
        $('.big_image p').text(thisText);
    });//.item.mouseenter()
